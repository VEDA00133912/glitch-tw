 ...
 gauge(config) {
    ...
    if (!cleared) {
      ...
      ctx.fillRect(
        x,
        firstTop,
        gaugeClear - x + 2 + (gaugeClear < gaugeW ? 0 : -7),
        22
      );
    }
    if (gaugeFilled > 0) {
    ...
    if (config.percentage === 1) {
      ...
      if (this.frameCount % 5 === 0) {
        this.animationFrameIndex =
          (this.animationFrameIndex + 1) % colors.length;
      }

      let totalLength =
        Math.min(gaugeW - 5, gaugeClear + 1, gaugeFilled - 4) +
        Math.min(gaugeW - 4, gaugeFilled - gaugeClear - 14);
      ...
      for (let i = 0; i < colors.length; i++) {
        gradient.addColorStop(
          1 - i / (colors.length - 1),
          colors[(i + this.animationFrameIndex) % colors.length]
        );
      }
      ...
      if (gaugeFilled > gaugeClear + 14) {
        ...
        let yellowGradient = ctx.createLinearGradient(
          0,
          secondTop,
          gaugeW,
          secondTop
        ...
        let glossGradient = ctx.createLinearGradient(
          0,
          secondTop,
          gaugeW,
          secondTop
        ...
        glossGradient.addColorStop(
          0,
          colors[this.animationFrameIndex % colors.length]
        );
        glossGradient.addColorStop(
          1,
          colors[(this.animationFrameIndex + 1) % colors.length]
        );
