class ParseTja {
  ...
  init(file, difficulty, stars, offset, metaOnly, mods) {
    ...
    for (let line of file) {
      ...
      if (
        indexComment !== -1 &&
        !line.trim().toLowerCase().startsWith('maker:')
      ) {
      ...
      if (line.slice(0, 1) === '#') {
        ...
        if (line.indexOf(':') > 0) {
          ...
          if (name === 'course') {
            ...
            value = value
              ? value.split(',').map((digit) => parseInt(digit))
              : [];
  ...
  parseCircles() {
    ...
    var checkChain = (note_chain, measure_length, is_last) => {
      ...
      for (var i = 0; i < note_chain.length - (is_last ? 1 : 0); ++i) {
        ...
        note.text =
          this.noteTypes_ex[note.type][
            alldon_pos != null ? (i - alldon_pos) % 2 : 0
          ];
      }
      //}
    };
        ...
        for (var i = 0; i < currentMeasure.length; i++) {
          ...
          if (note.type) {
            ...
            !(
              ...
              (!currentMeasure[i + 1] || currentMeasure[i + 1].type)
            ) &&
            !(
        ...
        switch (name) {
          ...
          case 'branchstart':
            ...
            branchObj = {
              ...
              active: active,
              type:
                value[0].trim().toLowerCase() === 'r' ? 'drumroll' : 'accuracy',
              requirement: req,
            };
            ...
            if (this.measures.length === 1 && branchObj.type === 'drumroll') {
              for (var i = circles.length; i--; ) {
                ...
                if (
                  (circle.endTime && circle.type === 'drumroll') ||
                  circle.type === 'daiDrumroll' ||
                  circle.type === 'balloon'
                ) {
                  this.measures.push({
                    ms: circle.endTime,
          ...
          case 'm':
            ...
            branchFirstMeasure = true;
            var branchName =
              name === 'm' ? 'master' : name === 'e' ? 'advanced' : 'normal';
            currentBranch = {
              name: branchName,
          ...
          switch (symbol) {
            ...
            case ',':
              if (
                ...
                (bpm !== lastBpm || gogo !== lastGogo || lyricsLine !== null)
              ) {
                insertBlankNote();
              }
              pushMeasure();
